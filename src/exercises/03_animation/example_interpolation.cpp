...
vec3 cardinal_interpolation(float t,float t0, float t1, float t2,float t3,const vec3& p0, const vec3& p1, const vec3& p2,const vec3& p3);
...
void scene_exercise::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    const float t0 = keyframe_time[(idx-1)%N];
    const float t1 = keyframe_time[idx];
    const float t2 = keyframe_time[idx+1];
    const float t3 = keyframe_time[(idx+2)%N];

    const vec3& p0 = keyframe_position[(idx-1)%N];
    const vec3& p1 = keyframe_position[idx];
    const vec3& p2 = keyframe_position[idx+1];
    const vec3& p3 = keyframe_position[(idx+2)%N];


    const vec3 p = cardinal_interpolation(t,t0,t1,t2,t3,p0,p1,p2,p3);
...
vec3 cardinal_interpolation(float t,float t0, float t1, float t2,float t3,const vec3& p0, const vec3& p1, const vec3& p2,const vec3& p3){
    float mu = 1;
    float s=(t-t1)/(t2-t1);
    vec3 d1=mu*(t2-t1)/(t2-t0)*(p2-p0);
    vec3 d2=mu*(t2-t1)/(t3-t1)*(p3-p1);
    vec3 res=(2*s*s*s-3*s*s+1)*p1;
    res+=(s*s*s-2*s*s+s)*d1;
    res+=(-2*s*s*s+3*s*s)*p2;
    res+=(s*s*s-s*s)*d2;
    return res;
}
